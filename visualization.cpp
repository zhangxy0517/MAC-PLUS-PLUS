...
void cloud_viewer_RGB(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, vector<Vertex>colors, int mode)
{
    int n = cloud->size();
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr est_gt(new pcl::PointCloud<pcl::PointXYZRGB>);
    vector<int>indices;
    indices.push_back(n-2);
    indices.push_back(n-1);
    pcl::PointIndicesPtr ind_ptr(new pcl::PointIndices);
    ind_ptr->indices = indices;

    if (mode){
        pcl::ExtractIndices<pcl::PointXYZRGB> extract;
        extract.setInputCloud(result);
        extract.setIndices(ind_ptr);
        extract.setNegative(false);
        extract.filter(*est_gt);
        extract.setNegative(true);
        extract.filter(*result);
    }
    //cout << est_gt->size() << " " << result->size() << " "<<n << endl;
    pcl::visualization::PCLVisualizer viewer("3D Viewer");
    viewer.setBackgroundColor(0, 0, 0);
    viewer.addPointCloud<pcl::PointXYZRGB>(result,  "sample cloud");
    viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "sample cloud");
    if(mode){
        viewer.addPointCloud<pcl::PointXYZRGB>(est_gt, "est_gt cloud");
        viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 6, "est_gt cloud");
    }
...
void RMSE_visualization(const PointCloudPtr& cloud_source, const PointCloudPtr&& cloud_target, Eigen::Matrix4f& Mat_est, Eigen::Matrix4f& Mat_GT, float mr)
{
	...
	cloud_viewer_RGB(cloud_source, RGB, 0);
}
